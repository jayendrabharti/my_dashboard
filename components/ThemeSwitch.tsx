...
const ThemeSwitch: React.FC<ThemeSwitchProps> = ({ className }) => {
  ...
  return (
    <button
      onClick={switchTheme}
      className={cn(
        "relative flex items-center justify-center w-10 h-10 rounded-full bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-colors duration-200",
        className
      )}
    >
      <Moon
